...
public class Downloader {
    ...
    public void download() {
        if (infos != null) {
            ...
            for (ThreadInfo info : infos) {
                ThreadManager.getThreadPool().execute(new MyThread(info.getThreadId(), info.getStartPos(), info.getEndPos(), info.getCompleteSize(), info.getUrl()));
            }
        }
    }
            ...
            try {
                ...
                if (responseCode == 200 || responseCode == 206) {
                    ...
                    while ((length = inStream.read(buffer, 0, buffer.length)) != -1) {
                        ...
                        List<FileStatus> fileStatuses = DataSupport.where("mUrl = ?", urlstr).find(FileStatus.class);
                        status.setCompleteSize(fileStatuses.get(0).getCompleteSize()+length);
